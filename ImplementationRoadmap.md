...
| LLM 묘사 데이터 | 프로토타입 10장과 strict 사건 투영 경계 작성 | 카드별 narration과 숨은 정보의 로컬 투영 검증 완료, 실제 프롬프트 조립과 출력 품질 검증은 남음 |
...
- `Tests/battle-controller-check.ps1`이 46개 호스트 경계 시나리오로 저장·View write-read 검증, 요청 주입, marker-only 무출력 재시도와 attempt 쓰기 유실, 미관측 출력 자동 삭제와 중단 재개, 관측 출력 보존·commit 복구를 검사한다.
- `Tests/main-hook-check.ps1`이 `editInput`, `onStart`, `editRequest`, `onOutput`과 기존 버튼 라우터의 성공·실패 분기를 두 독립 Lua 프로세스에서 검사한다.
