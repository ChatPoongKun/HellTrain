...
$contractPath = Join-Path $root 'StateViewContract.md'
...
$route = 'risu-btn="battleController|clickCard|{{dictelement::{{slot::hand_card}}::instanceId}}|{{dictelement::{{tempvar::bv}}::interactionToken}}"'
Assert-Contract ($body.Contains($route)) 'card click route does not include instanceId and interactionToken'
...
Assert-Contract ([regex]::Matches($body, 'risu-btn=').Count -eq 1) 'battle UI exposes an unexpected button route'
...
Assert-Contract ($body.Contains('data-focused="1"')) 'focused state is not bound to card markup'
