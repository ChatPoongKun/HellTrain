...
function LLM(_,prompt)
 ...
 assert(prompt[1].content:find('"sexualPreference"',1,true),'sexual preference missing from compact relationship context')
 assert(prompt[1].content:find('"backgroundNarrative"',1,true),'background narrative missing from compact relationship context')
 assert(not prompt[1].content:find('"privateProfile"',1,true),'legacy private profile leaked into compact relationship context')
