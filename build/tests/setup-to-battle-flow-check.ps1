...
local lorePaths = {
    ...
    ["battleui-interaction.html"] = "html/battleui-interaction.html",
}
...
local UI_INTERACTION = "helltrainBattleInteractionV1"
local RUNTIME_KEYS = {
...
print("DEBUG_SELECTED=" .. canonical(selected))
assert(receipt.phase == "battleReady"
        ...
        and type(receipt.battleSpec.environmentId) == "string",
...
assert(chatVars[UI_INTERACTION] == readFile("html/battleui-interaction.html"),
    "battle handoff did not publish the interaction fragment")
assert((loreLoads["battleui.html"] or 0) == 1,
    "initial battle UI was not loaded exactly once")
assert((loreLoads["battleui-interaction.html"] or 0) == 1,
    "initial battle interaction UI was not loaded exactly once")
...
assert((moduleActions["battleController._startFromCanonicalSetup"] or 0) == 1,
...
local function assertRetryPreserved(label, report, expectedStale)
    ...
    assert(chatVars[UI_INTERACTION] == readFile("html/battleui-interaction.html"),
        label .. " did not preserve battleui-interaction.html")
...
assert((moduleActions["battleController._startFromCanonicalSetup"] or 0) == 4,
