...
def player_prototype_blocks(path: Path) -> list[tuple[str, str, str]]:
    text = path.read_text(encoding="utf-8")
    starts = list(re.finditer(r'(?m)^add(?:\(\{ |\{)id\s*=\s*"(pc_[a-z]+_\d{3})"', text))
    blocks = []
    for index, match in enumerate(starts):
        end = starts[index + 1].start() if index + 1 < len(starts) else len(text)
        blocks.append((match.group(1), "prototype", text[match.start():end]))
    return blocks
...
def player_header(block: str, constructor: str) -> tuple[list[str], str, list[str]]:
    if constructor == "prototype":
        roles_match = re.search(r'\broles\s*=\s*(\{[^}]+\})', block, re.S)
        type_match = re.search(r'\bcardType\s*=\s*"([a-z][a-z0-9_]*)"', block)
        mechanisms_match = re.search(r'\bmechanisms\s*=\s*(\{[^}]*\})', block, re.S)
        return (
            quoted_ids(roles_match.group(1)) if roles_match else [],
            type_match.group(1) if type_match else "action",
            quoted_ids(mechanisms_match.group(1)) if mechanisms_match else [],
        )
...
def main() -> int:
    ...
    for card_id, constructor, block in player:
        ...
        if constructor == "prototype":
            damage_match = re.search(r'\bdamage\s*=\s*(\d+)', block)
        else:
        ...
        base_damage = int(damage_match.group(1)) if damage_match and constructor in {"card", "prototype"} else 0
