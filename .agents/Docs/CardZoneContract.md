...
`placePlan`의 `planSpec`은 `durationTurns`, `charges`, `durationIncludesPlacementTurn`, `revealed`만 허용하며 지속시간이나 충전 중 하나 이상은 양의 정수여야 한다. `durationIncludesPlacementTurn`은 선택 boolean이고 생략하면 `false`이며, `true`로 쓰려면 `durationTurns`가 반드시 있어야 한다. 이 값은 점유 슬롯에도 같은 이름으로 보존하되, 생략된 값은 `false`와 동일하게 처리한다.

정적 카드 계약이 허용하는 `expires` 함수만의 계획은 아직 JSON 런타임 표식과 공통 폐기 action이 확정되지 않았다. 카드 영역 v1은 이런 계획을 배치하지 않으며, 현재 프로토타입 계획들처럼 `durationTurns` 또는 `charges`가 있는 정의만 지원한다.

- 새 계획을 놓으면 기존 계획은 `discard`로 이동한다.
- `durationIncludesPlacementTurn`이 생략됐거나 `false`면 배치 턴에는 지속시간을 감소시키지 않고 다음 턴부터 턴 종료에 1 감소시킨다.
- `durationIncludesPlacementTurn = true`면 배치한 턴 종료부터 지속시간을 1 감소시킨다.
...
`.agents/Tests/deterministic-rng-check.ps1`과 `.agents/Tests/card-zones-check.ps1`은 로컬 결정성, 카드 보존과 10턴 영역 순환을 검증한다. 실제 RisuAI 로어북 호출은 별도 통합 단계에서 확인한다.
