...
이 문서는 정적 Lua 카드 DB, 전투 엔진, 세이브 상태, UI View와 LLM 사건 사이의 경계를 정의한다. 아래 열 장은 밸런스나 최종 콘텐츠가 아니라 데이터 구조와 엔진 경로를 검증하기 위한 프로토타입이다.
...
local planState = {
    ...
    remainingTurns = 1,
    remainingCharges = 1,
    durationIncludesPlacementTurn = false,
    revealed = false,
}
```

`durationIncludesPlacementTurn`은 선택 boolean이며 생략하면 `false`다. 정적 카드 함수를 런타임 상태에 복사하지 않는 원칙과 달리, 이 값은 턴 종료 수명 계산에 필요한 단순 설정이므로 계획 배치 명세와 점유 슬롯에 보존할 수 있다.
...
| `selectionPreview` | 선택 | 같은 턴 선택을 확장하는 선언형 선택 단계 효과 |
...
같은 턴에 드로우한 카드를 다시 등록할 수 있어야 하는 플레이어 `chain` 카드는 일반 해결보다 앞선 선언형 `selectionPreview.effects`를 사용할 수 있다.
...
이 값은 화면용 힌트가 아니라 선택 단계 효과의 단일 원본이다. `turnDraft`는 권위 상태의 복제본에 이 명령을 적용해 프리뷰를 만들고, 전송 projection에서도 같은 정의를 다시 검증해 한 번만 적용한다. v1은 플레이어 연계 카드의 `draw_cards`만 허용하며 `selectionPreview`와 일반 `resolve`를 한 카드에 함께 두지 않는다.

선택 단계에서는 일반 `resolve`, 환경, 특징, 상대 계획, 캐릭터 의도와 카드 선언 트리거를 실행하지 않는다. 프리뷰 취소는 권위 덱과 RNG를 바꾸지 않으며 같은 상태에서 재등록하면 같은 카드를 보여준다. 상세 상태 전이는 `TurnDraftContract.md`를 따른다.
...
선택 단계 효과가 있는 카드는 전송 projection에서 해당 효과와 등록 카드 영역 이동을 먼저 한 번 적용한다. 후속 실제 카드 해결기는 그 workingState를 이어받으며 같은 선택 단계 효과를 다시 적용하지 않는다. 그 뒤 카드 한 장은 다음 순서로 해결한다.
...
계획 데이터는 `durationTurns`, `charges`, `expires` 중 하나 이상으로 수명이 제한되어야 한다. `durationTurns`와 `charges`를 사용한다면 1 이상의 정수여야 한다. 선택 필드 `durationIncludesPlacementTurn`은 boolean이며 생략하면 `false`다. 이 값을 `true`로 쓰려면 `durationTurns`가 반드시 있어야 한다.

`durationIncludesPlacementTurn = false`인 기본 계획은 배치한 턴에는 지속시간을 감소시키지 않고 다음 턴 종료부터 감소한다. `true`인 계획은 배치한 턴 종료부터 감소한다. 어느 경우든 턴 종료 감소로 남은 지속시간이 0이 되면 즉시 버린 카드 더미로 이동한다. 엔진은 이 값을 정적 DB에서 계획 배치 명세와 점유 슬롯으로 전달하며, 생략된 값은 모든 경계에서 `false`와 같은 의미로 처리한다.
...
return {
    ...
    cards = {
        ...
        persistent_press = {
            ...
            owner = "player",
            name = "대담한 밀착",
            description = "::tag[contact]:: 행동으로 거리를 단숨에 좁혀 상대를 강하게 압박하고 저항을 낮춥니다.",
            actionTag = "contact",
            mechanisms = {},
            base = {
                stealthCost = 2,
            ...
            narration = {
                play = {
                    actorAction = "망설임 없이 거리를 좁혀 상대를 강하게 압박한다.",
                },
            },
            prototype = true,
        },
        ...
        dangerous_whisper = {
            id = "dangerous_whisper",
            owner = "player",
            name = "위험한 속삭임",
            description = "::tag[threat]:: 행동으로 낮은 목소리로 위협하며, 혼란한 상대를 더욱 강하게 압박합니다.",
            actionTag = "threat",
            mechanisms = {},
            base = {
                stealthCost = 1,
            ...
            rules = {
                "혼란 무드라면 저항을 추가로 4 낮춥니다.",
            },
            moodEffects = {
                confusion = function(context)
                    return {
                        {
            ...
            narration = {
                play = {
                    actorAction = "다른 승객에게 들리지 않도록 낮은 목소리로 위협적인 말을 건넨다.",
                },
            },
            prototype = true,
        },
        ...
        cut_off_escape = {
            ...
            owner = "player",
            name = "퇴로 봉쇄",
            description = "::tag[approach]:: 행동으로 상대의 이동 경로를 선점하고, 상대 행동 뒤 발동하는 ::tag[plan]::을 배치합니다.",
            actionTag = "approach",
            mechanisms = { "plan" },
            base = {
                stealthCost = 1,
            ...
            narration = {
                planPlaced = {
                    actorAction = "상대가 움직일 방향을 읽고 먼저 자리를 잡아 퇴로를 막는다.",
                },
                planTriggered = {
                    actorAction = "상대가 움직인 직후 막아 둔 방향으로 다시 압박한다.",
                },
            },
            prototype = true,
        },
        ...
        cross_the_line = {
            ...
            owner = "player",
            name = "선을 넘다",
            description = "::tag[violation]:: 행동으로 돌이킬 수 없는 수준까지 압박하며, 사용 후 ::tag[remove]::되어 이번 세션에서 제외됩니다.",
            actionTag = "violation",
            mechanisms = { "remove" },
            base = {
                stealthCost = 8,
            ...
            rules = {
                "순응 무드라면 저항을 추가로 5 낮춥니다.",
                "순응 무드가 아니라면 무드를 거절로 직접 설정하고 턴 종료까지 고정합니다.",
                "사용 후 이번 세션에서 ::tag[remove]::됩니다.",
            },
            resolve = function(context)
            ...
            narration = {
                play = {
                    actorAction = "돌이킬 수 없을 만큼 강하게 상대의 경계를 밀어붙인다.",
                },
            },
            prototype = true,
        },
...
- 계획에 제한된 수명이 있는지 검사
- `durationIncludesPlacementTurn`이 boolean인지, `true`일 때 `durationTurns`가 함께 있는지 검사
...
다음 항목은 해당 콘텐츠 또는 통합 단계에 들어가기 전에 공동 결정한다.

- 일반 비용·피해 보정 콘텐츠를 추가할 때의 가산, 배율, 최소값과 반올림 순서. 현재 v1은 기본값과 명시 효과값만 사용한다.
- 카드 콜백이 현재 이벤트와 과거 이벤트를 어느 범위까지 읽을 수 있는지
- `expires` 함수만으로 수명이 정해지는 계획의 JSON 런타임 표식과 공통 폐기 action. 현재 카드 영역 v1은 `durationTurns` 또는 `charges`가 있는 계획만 배치할 수 있음
