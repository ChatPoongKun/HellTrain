...
- RisuAI가 들여쓴 HTML/CSS/CBS 줄을 코드 블록이나 잘못된 출력으로 해석할 수 있으므로, 실제 게임용 `html/*.html`의 모든 줄은 공백이나 탭 없이 첫 열에서 시작한다. Lua가 `.html` 로어를 조립할 때도 각 줄의 선행 공백과 탭을 제거한다.
...
## 기타 주요사항.
- 리비전 갱신을 위한 목적 외에는 명시적 지시 없이 main.lua파일을 수정해서는 안됨.
