...
캐릭터 카드 정의는 각 `Char/<캐릭터>.db`의 `cards`에 저장하며, 해당 캐릭터의
`battle.deck`은 같은 DB의 카드 ID를 참조합니다. 모든 캐릭터가 동일한 덱 규칙을 사용하며,
전투 시작 시 `battle.deck`이 10장 미만이면 시드에 따라 공용 풀에서 10장이 되도록
중복 없이 자동 보충합니다. 10장 이상이면 전용 카드만 사용합니다. 별도의 카드 풀 opt-in
필드는 필요하지 않습니다. 상세 작성법은
...
승리 흐름은 즉시 정산되지 않고 `aftermath` 상태에서 남은 이동과 최소 2회의 자유행동 출력을 처리합니다. 마지막 장면이 완료되면 전투 로그와 진행 상태를 갱신하고 다음 선택 화면 또는 종료 화면으로 이동합니다.
...
카드 선택·선택 확정 버튼은 전용 경량 경로를 사용합니다. 선택 중에는 현재 턴·토큰·사용 조건과 드로우 프리뷰를 확인하며, 전체 전투 상태·이력·지문·프리뷰의 정합성 검사는 실제 전송 시 수행합니다. `battleInteractionUi.lua`가 선택 View에서 HTML을 직접 조립하므로 클릭마다 HTML 로어와 CBS를 재평가하지 않습니다. 초기·전체 화면은 기존 HTML 템플릿을 사용하며, 손패 표시를 바꿀 때에는 `html/battleui-interaction.html`과 Lua 렌더러의 버튼·팝오버·표시 동작을 함께 유지해야 합니다. 도감은 새 프리뷰 카드가 공개될 때만 갱신하고, 선택 취소 후에도 발견 기록을 유지합니다.
